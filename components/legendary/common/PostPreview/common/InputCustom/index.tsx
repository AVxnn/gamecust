...
const InputCustom = ({value, statusBar, onChange, error, type, placeholder} : any) => {
    ...
    return (
        <div className={styles.formInput} onFocus={() => setFocus(true)} onBlur={() => setFocus(false)}>
            ...
            {
                type === 'password' ? <div className={styles.eye} onClick={() => typeS === 'password' ? setTypeS('text') : setTypeS('password')}><Eye /></div> : ''
            }
            {
                type === 'password' && statusBar && focus ? <div className={styles.line}><div className={styles.activeLine} style={{backgroundColor: '#9E5BF0', width: `${statusBar}%`}}></div></div> : ''
            }
        </div>
    )
}
